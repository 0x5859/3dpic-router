...
namespace Catch {
    ...
    // Vendored patch: the legacy `int $3` inline asm is x86-only and
    // fails to assemble on Apple Silicon (clang errors with
    // "unrecognized instruction mnemonic"). Fall back to the portable
    // __builtin_trap() on aarch64 — matches the arm64 adjustments the
    // README documents for lbfgs-cpp's `emplace_back_kernel_8`.
